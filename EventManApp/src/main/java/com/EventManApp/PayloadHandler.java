...
public class PayloadHandler extends CommandHandler{
    ...
    public PayloadHandler(KVObjectHandler kvObjectHandler, KVSubjectHandler kvSubjectHandler) {
        super();
        //DebugUtil.debug();
    ...
    public boolean isValidCommand(String subjectId, String commandId) {
        //return commandMap.containsKey(commandId);
        return kvSubjectHandler.getKVSubject(subjectId) != null;
    }


    private String[] extractCommandId(String commandId) {
        // Return null parts if commandId is null or empty
        if (commandId == null || commandId.isEmpty()) {
            return new String[]{null, null}; // Return null for both parts
        }

        String[] parts = commandId.split("\\.");
        String commandPart = parts[parts.length - 1]; // Always the last part
        String subjectPart = (parts.length > 1) ? String.join(".", Arrays.copyOf(parts, parts.length - 1)) : null;

        return new String[]{subjectPart, commandPart};
    }
    ...
    public JSONObject parsePayload(String jsonPayload) {
        try {
            ...
            String subjectId = payload.getIdentifier();
            if (subjectId == null) {
                return ResponseHelper.createResponse("Invalid payload: Missing identifier.", null);
            }
            
            if (refineCommands(subjectId, payload.getCommands())) {
                return executeCommands(payload); // Return the output of executeCommands
            }
   ...
   private boolean checkResponseMessage(JSONObject item, String key) {
    ...
    private boolean refineCommands(String identifier, List<PayloadCommand> commands) {
        //DebugUtil.debug(identifier, commands.toString());
        ...
        for (PayloadCommand command : commands) {
            ...
            String[] extractedCommand = extractCommandId(command.getId()); // Assuming payload has an Id
            commandId = extractedCommand[1];
            if (extractedCommand[0] != null){
                subjectId = extractedCommand[0];
            }

            if (commandId != null && isValidCommand(subjectId,commandId)) {
                Map<String, PayloadDetail> args = command.getArgs();
                Map<String, Object> data = command.getData();

                KVSubject subject = kvSubjectHandler.getKVSubject(subjectId);
                if (subject == null) {
                    ...
                    for (Map.Entry<String, PayloadDetail> entry : args.entrySet()) {
                        ...
                        if ("auto".equals(fieldDetail.getModifier())) {
                            ...
                            if (value == null || "".equals(value)) {
                                value = fieldDetail.getDefaultValue();
                            }
                       }
    ...
    private JSONObject executeCommands(Payload payload) {
        ...
        for (PayloadCommand command : commands) {
            JSONObject commandResponse = executeCommand(command);
    ...
    private KVObject createKVObject(String identifier, Object data) {
        // Retrieve the field type map for KVObject
        Map<String, KVObjectField> fieldTypeMap = kvSubjectHandler.getFieldTypeMapByIdentifier(identifier);
    ...
    public JSONObject addRow(String identifier, PayloadCommand command) {
        if (command.getData() != null) {
            try {
                ...
                KVObject kvObject = createKVObject(identifier, command.getData());

                if (kvObject != null) {
                    // Check if the KVObject was successfully added
                    boolean addedSuccessfully = kvObjectHandler.addKVObject(kvObject);
        ...
        return kvObject -> {
            ...
            for (String fieldName : fieldNames) {
                ...
                String compareStr = (data != null && data.containsKey(fieldName)) 
                    ...
                    : (args.containsKey(fieldName) ? args.get(fieldName).getDefaultValue() : null);
    ...
    public JSONObject removeRow(String identifier, PayloadCommand command) {
        ...
        try {
            ...
            boolean removed = kvObjectHandler.removeKVObject(identifier, validator);
    ...
    public JSONObject getRow(String identifier, PayloadCommand command) {
        try {
            ...
            KVObject kvObject = kvObjectHandler.getKVObject(identifier, validator);
            ...
            if (kvObject != null) {
                ...
                return ResponseHelper.createResponse("Successfully retrieved the KVObject.", 
                    new JSONObject().put("id", kvObject.getIdentifier())
                                    .put("data", kvObject.toJSON())); // Adjust the data as needed
            }
        } catch (Exception e) {
            System.out.println(e.getMessage());
    ...
    public JSONObject getRows(String identifier, PayloadCommand command) {
        try {
            ...
            List<KVObject> kvObjects = kvObjectHandler.getKVObjects(identifier, validator);
